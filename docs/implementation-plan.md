...
`atom` が扱える拡張子に絞り、270件で比較した。MarkItDownが `ok` で
`atom` が非成功のケースは0件になった。`ok` / `ok` の263件では、
MarkItDown合計698,797msに対し `atom` は425,048msだった。
一方でMarkItDown勝ち107件の遅延合計59,142msのうち、PDFが82件、
57,395msを占めた。最大外れ値は `31_67.pdf` であり、PDFバックエンドは
M6の残タスクとして速度外れ値を改善する。
